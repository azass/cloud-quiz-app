...
const conditionLetters = ["今日", "昨日", "2日前", "3日前", "4日前", "5日前", "6日前", "1週間以上前", "2週間以上前", "1ヶ月以上前"]

export const QExcludeButton: VFC<Props> = memo(({ index, execute_progress }) => {
  const { selectExcludeDays } = useContext(SearchContext)
  ...
  const count = () => {
    if (index < 7) {
      ...
      return execute_progress.slice(7, 14).reduce((sum, count) => sum.concat(count)).length
    } else if (index === 8) {
      return execute_progress.slice(14).reduce((sum, count) => sum.concat(count)).length
    } else if (index === 9) {
      return questions.filter((question) => question && !question.is_old).length - execute_progress.reduce((sum, count) => sum.concat(count)).length
    } else {
      return 0
    }
  }
